...
import 'package:challenge/features/measurments/ui/widgets/health_measurment_buttons.dart';
import 'package:challenge/features/measurments/ui/widgets/part_screen_pressure.dart';
import 'package:challenge/features/measurments/ui/widgets/part_screen_suger.dart';
import 'package:challenge/features/measurments/ui/widgets/part_screen_weight.dart';
...
class _MyAppState extends State<MeasurementsScreen> {
  ...
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (context) => getit<MeasurmentsCubit>(),
      child: BlocConsumer<MeasurmentsCubit, MeasurmentsState>(
        listener: (context, state) {
          state.maybeWhen(
            addWeightError: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('حدث خطأ أثناء حفظ الوزن.')),
              );
            },
            addBloodPressureError: () {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(content: Text('حدث خطأ أثناء حفظ ضغط الدم.')),
              );
            },
                  ...
                  SliverToBoxAdapter(
                    child: Row(
                      ...
                      children: [
                        ...
                        HealthMeasurementButton(
                          imagePath:
                              'assets/images/icon-park-outline_weight.png',
                          label: "وزن",
